...
let handler = async function (message, { conn, text, command, isAdmin }) {
    try {
        if (command === 'الالقاب') {
            ...
            if (!isAdmin) {
                message.reply('معذره هذا الامر يمكن استخدامه فقط المشرفين🥲');
                return;
            }
            const nicknames = await BK9.find({ groupId: message.chat });
            if (nicknames.length === 0) {
                message.reply(' لا يـوجـد الـقـاب مـسـجـلـة حــالــيــاً ┇');
            } else {
                let replyText = '';
            ...
            result.deletedCount > 0 ? message.reply('┇ تم حذف اللقب ' + nickname + ' بنجاح') : message.reply('اللّقب ' + nickname + ' غير مسجل لاحد ');
            ...
            if (userNickname) {
                ...
                message.reply('┇ اللقب ' + nickname + ' مأخوذ من طرف @' + userName);
            } else {
