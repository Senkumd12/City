...
const handler = async (m, {conn, usedPrefix, text}) => {
  ...
  const textquien = `*من تريد ان تتحدى؟* \n *اعمل منشن لشخص*\n\n *❐↞┇مثال ↞ .تحدي @${global.suittag}┇*
  `;
  ...
  const caption = `*🎮 حجر ورقة مقص 🎮*\n\n—◉ @${m.sender.split`@`[0]} لقد تحداك هذا الشخص في جولة @${m.mentionedJid[0].split`@`[0]} \n◉ اكتب "قبول" للموافقة\n◉ اكتب"رفض" للرفض \nيجب ان ترد علي هذه الرسالة`;
  ...
  conn.suit[id] = {
    ...
    status: 'wait',
    waktu: setTimeout(() => {
      if (conn.suit[id]) conn.reply(m.chat, `*❐┃لـقـد انـتـهـى الـوقـت┃⌛ ❯*`, m);
